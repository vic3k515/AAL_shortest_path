...
void runAlgorithms(Raster* rptr, bool benchmark, vector<int>* bench_info=nullptr)
{
	...
	int duration;
	typedef vector<Raster::Location> Path;
	...
	{
		/* run algorithm 5 times and save each exec time in bech_info */
		for(int i=0; i<5 ; ++i)
		{
		...
		bfs.shortestPath(start, end);
	Path shortest_path_1 = bfs.getPath();
	if(!benchmark)
	...
	if (benchmark && bench_info != nullptr)
	{
		...
		dijkstra.shortestPath(start, end);
	Path shortest_path_2 = dijkstra.getPath();
	if(!benchmark)
	...
	if (benchmark && bench_info != nullptr)
	{
		...
		a_star.shortestPath(start, end);
	Path shortest_path_3 = a_star.getPath();
	if(!benchmark)
...
void benchmarkMode()
{
	...
	vector<double> bfs_q, dijkstra_q, a_star_q,bfs_times, dijkstra_times, a_star_times;

	int median;
	double T_med, T, constant, v_med, e_med;

	// table formatting related variables
	const char fill = ' ';
	const int width = 12;

	// run algorithms and collect benchmark data
	...
	for (auto it = results.begin(); it != results.end(); )
	{
		...
		for(int i=0; i<5; ++i)
			time_sum += *it++;
		bfs_times.push_back(((double)time_sum)/5.0);
		time_sum=0;
		for(int i=0; i<5; ++i)
			time_sum += *it++;
		dijkstra_times.push_back(((double)time_sum)/5.0);
		time_sum=0;
		for(int i=0; i<5; ++i)
			time_sum += *it++;
		a_star_times.push_back(((double)time_sum)/5.0);
	}
	...
	v_med = (double)vertices[median];
	e_med = (double)edges[median];

	// BFS - O(V+E)
	T_med = v_med + e_med;
	constant = T_med / bfs_times[median];
	bfs_q[median] = 1.0;
	for (int i = 0; i < no_test; ++i)
	{
		if (i == median) continue;
		T = (double)(vertices[i] + edges[i]);
		bfs_q[i] = (bfs_times[i] * constant) / T;
	}

	// Dijkstra - O(E*logV)
	T_med = e_med * log((double)v_med);
	constant = T_med / dijkstra_times[median];
	...
	// A* - O(E)
	T_med = e_med;
	constant = T_med / a_star_times[median];
	a_star_q[median] = 1.0;
	for (int i = 0; i < no_test; ++i)
	{
		if (i == median) continue;
	...
	for (int k = 0; k < 3; ++k)
	{
		...
		cout << left  << setw(width) << setfill(fill) << "t(n)[us]";
		...
		for (int i = 0; i < no_test; ++i)
		{
			...
			if(k==0)
				cout << std::left << setprecision(1) << std::fixed << setw(width) << setfill(fill) << bfs_times[i];
			else if(k==1)
				cout << std::left << setprecision(1) << std::fixed << setw(width) << setfill(fill) << dijkstra_times[i];
			else
				cout << std::left << setprecision(1) << std::fixed << setw(width) << setfill(fill) << a_star_times[i];
			if(k==0)
				cout << std::left << setprecision(3) << std::fixed << setw(width) << setfill(fill) << bfs_q[i] << endl;
			else if(k==1)
				cout << std::left << setprecision(3) << std::fixed << setw(width) << setfill(fill) << dijkstra_q[i] << endl;
			else
				cout << std::left << setprecision(3) << std::fixed << setw(width) << setfill(fill) << a_star_q[i] << endl;
		}
	}
}
